namespace GHI.PythonSharp.Compiler
{
    ...
    public class Parser
    {
        ...
        public ArrayList CompileExpressionList()
        {
        ...
        {
            return token != null && token.TokenType == TokenType.Operator && Strings.Contains(opslevel0, token.Value);
        }

        private static bool IsLevel1Operator(Token token)
        {
            return token != null && token.TokenType == TokenType.Operator && Strings.Contains(opslevel1,token.Value);
        }

        private static bool IsLevel2Operator(Token token)
        {
            return token != null && token.TokenType == TokenType.Operator && Strings.Contains(opslevel2,token.Value);
        }

        private static bool IsLevel3Operator(Token token)
        {
            return token != null && token.TokenType == TokenType.Operator && Strings.Contains(opslevel3,token.Value);
        }
        ...
        private ExpressionCommand CompileExpressionCommand()
        {
            ...
            if (list.Count == 1)
                return new ExpressionCommand(list[0] as IExpression);
        ...
        private ICommand CompileSimpleCommand()
        {
            ...
            if (token.Value == "from")
            {
                ...
                var names = this.CompileNameList();
        ...
        private ArrayList CompileNameList()
        {
        ...
        private ArrayList CompileParameterExpressionList()
        {
        ...
        private ICommand CompileDefCommand()
        {
            ...
            this.CompileToken(TokenType.Separator, "(");
            var parameters = this.CompileParameterExpressionList();
            this.CompileToken(TokenType.Separator, ")");
        ...
        private ArrayList CompileArgumentExpressionList()
        {
            var expressions = new ArrayList();
        ...
        {
            var expressions = this.CompileArgumentExpressionList();
            this.CompileToken(TokenType.Separator, ")");
        ...
        private IExpression CompileSimpleTerm()
        {
            ...
            switch (token.TokenType)
            {
                ...
                case TokenType.Boolean:
                    return new ConstantExpression(ConvertExtensions.ToBoolean(token.Value));
                case TokenType.Name:
                ...
                case TokenType.Separator:
                    if (token.Value == "(")
                    {
                        ...
                        if (list == null)
                            ...
                            return new ListExpression(new ArrayList(), true);

                        if (list.Count == 1)
                            return list[0] as IExpression;
